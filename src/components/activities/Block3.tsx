...
export function Block3() {
  ...
  const submitA1 = () => {
    ...
    const pts = Math.min(5, correctByObj + (goodSpread ? 1 : 0) + (Object.keys(a1Priorities).length >= 5 ? 1 : 0))
  ...
  return (
    <div>
      {/* A9: Platform Selection */}
      <ActivityCard number={9} title="Platform Selection" subtitle="Set priority levels for Nike across each platform — feeds your Agency Pitch" points={scores.b3a1?.points || 0} locked={a1Locked} isPitch>
        {objectives.length > 0 && (
          <div className="bg-brand-50 border border-brand-200 rounded-xl p-3 mb-4 text-xs text-brand-700">
            <strong>Your objectives:</strong> {objectiveLabels}
            {objectives.map(id => {
        ...
        <Alert type="info">📊 Assign a priority to each platform for Nike's 18–24 female campaign. Aim for 2–3 Primary platforms — spreading resource too thin hurts performance.</Alert>
            ...
            return (
              <div key={platform.id} className={`border-2 rounded-xl p-3 transition-all ${a1Locked && isObjFit && pick === 'Primary' ? 'border-emerald-300' : 'border-slate-200'}`}>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {isObjFit && !a1Locked && <span className="text-[10px] text-brand-500 font-semibold">Aligns with objectives</span>}
